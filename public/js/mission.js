            ...
            if (index >= 1) {
                ...
                const stepBox = $("<div>")
                    ...
                    .hover(
                        e => {
                            stepBox.html(
                                '<svg xmlns="http://www.w3.org/2000/svg" width="50px" height="50px" viewBox="0 0 320 512"><path d="M294.6 166.6L317.3 144 272 98.7l-22.6 22.6L160 210.7 70.6 121.4 48 98.7 2.7 144l22.6 22.6L114.7 256 25.4 345.4 2.7 368 48 413.3l22.6-22.6L160 301.3l89.4 89.4L272 413.3 317.3 368l-22.6-22.6L205.3 256l89.4-89.4z"/></svg>'
                            )
                            stepBox.css({
                                "transition-duration": "0.3s",
...
async function uploadStudentDecideSteps() {
    ...
    await axios.all([uploadDecide(), uncompleteNext()]).then(
...
function stepsSelectOption() {
    ...
    function disableBlockandOptions() {
        ...
        $(".optionDecideContent").fadeOut(200)
        setTimeout(() => {
            $(".container-fluid").remove()
            $(".optionDecideContent").remove()
        }, 500)
    }
...
function dragMission() {
    ...
    function getDragAfterElement(container, y) {
        const draggableDiv = [
            ...container.querySelectorAll(".missionText:not(.dragging)"),
        ]

        return draggableDiv.reduce(
            (closest, child) => {
    ...
    //本周已選項目(拉動任務欄後)
    function renderMissionDeicdewhenDragging(
        currentStepOptions,
        currentMissions
    ) {
...
function renderMissionPage(data) {
    ...
    data.studentSelect
        ? renderMissionDecidewhenLoading(data.studentSelect)
        : null
